...
const int SEP        = 5000;     // Separation between configurations
const int THERMALIZE = 50000;   // Number of updates for thermalization
...
void Grow_cluster( ) {
  ...
  double z   = 2.0 * dist( engine ) - 1.0;     // z-component 
  double phi = 2.0 * M_PI * dist( engine );    // Azimuthal angle
  double sintheta = sqrt( 1.0 - z * z );       // sin( theta ) 

  probe_spin[0] = sintheta * cos( phi );       // x
  probe_spin[1] = sintheta * sin( phi );       // y
  probe_spin[2] = z;                           // z
...
int main(){
  ...
  ifstream field_check( "/Users/markbook/Desktop/LOCAL/NLSM_TESTS/L128/0_Configurations/field_last_new2.dat" ); // Call the file with the last
  ...
  if( status ){
    ...
    field_last.open( "/Users/markbook/Desktop/LOCAL/NLSM_TESTS/L128/0_Configurations/field_last_new2.dat" ); // Opens the file
  ...
  field_confs.open( "/Users/markbook/Desktop/LOCAL/NLSM_TESTS/L128/0_Configurations/field_new2.dat", ios_base::app ); // the information of the
  ...
  ofstream field_bckp;
  field_bckp.open( "/Users/markbook/Desktop/LOCAL/NLSM_TESTS/L128/0_Configurations/field_last_new2.dat" );
  for ( int t = 0; t < TIME; t++) {
    for ( int x = 0; x < LEN; x++) {
      angles = Get_angles( field[t][x] );
      field_bckp <<fixed<<showpoint<<setprecision(16)<<angles[0]<<","
                 <<fixed<<showpoint<<setprecision(16)<<angles[1];
